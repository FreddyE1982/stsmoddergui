...
LOCALIZATION_MANIFEST_FIXED_FIELDS = {
    "category",
    "field",
    "identifier",
    "originalLanguage",
...
def _format_localization_path(path: Tuple[PathSegment, ...]) -> str:
    if not path:
        return ""
    return "/".join(str(segment) for segment in path)


def _parse_localization_path(path: str) -> Tuple[PathSegment, ...]:
    if not path:
        return ()
...
def _walk_localization_payload(
    payload: Any,
    *,
    prefix: Tuple[PathSegment, ...] = (),
) -> Iterator[Tuple[Tuple[PathSegment, ...], str]]:
    ...
    if isinstance(payload, Mapping):
        for key in sorted(payload):
            ...
            if value is None:
                continue
            yield from _walk_localization_payload(value, prefix=prefix + (str(key),))
        return
    if isinstance(payload, Sequence) and not isinstance(payload, (str, bytes, bytearray)):
        for index, value in enumerate(payload):
            if value is None:
                continue
            yield from _walk_localization_payload(value, prefix=prefix + (index,))
...
class LocalizationKey:
    ...
    def field(self) -> Optional[str]:
        ...
        for segment in tail:
            if isinstance(segment, int):
                if formatted:
                    formatted[-1] = f"{formatted[-1]}[{segment}]"
                else:
    ...
    def category(self) -> str:
        ...
        stem = Path(name).stem
        return stem or "localization"

    def as_path_string(self) -> str:
        return _format_localization_path(self.path)


@dataclass(frozen=True)
class LocalizationManifestEntry:
    key: LocalizationKey
...
def _serialise_localization_manifest(entries: Sequence[LocalizableTextEntry]) -> str:
    if not entries:
        header = [
            "# No localizable texts were discovered for this build.",
            "# Add cards, relics or other assets to populate this manifest.",
        ]
        return "\n".join(header) + "\n"

    lines = [
        "# Localization manifest generated by compile_and_bundle.",
        "# Add translations by introducing language codes under each entry.",
        "# Values use JSON-style quotes so special characters stay intact.",
    ]
    for entry in entries:
        lines.append(f"- category: {_quote_yaml_string(entry.category)}")
        lines.append(f"  source: {_quote_yaml_string(entry.key.source)}")
        lines.append(f"  path: {_quote_yaml_string(entry.key.as_path_string())}")
        if entry.identifier is not None:
            lines.append(f"  identifier: {_quote_yaml_string(entry.identifier)}")
        if entry.field is not None:
            lines.append(f"  field: {_quote_yaml_string(entry.field)}")
        lines.append(f"  originalLanguage: {_quote_yaml_string(entry.original_language)}")
        lines.append(f"  originalText: {_quote_yaml_string(entry.original_text)}")
        for language in sorted(entry.translations):
            lines.append(f"  {language}: {_quote_yaml_string(entry.translations[language])}")
    lines.append("")
    return "\n".join(lines)
        ...
        for line_number, raw_line in enumerate(handle, start=1):
            ...
            if current is None:
                raise BaseModBootstrapError(
                    f"Invalid localisation manifest at {path}:{line_number}: entries must start with '-'."
                )
            if ":" not in stripped:
                raise BaseModBootstrapError(
                    f"Invalid localisation manifest at {path}:{line_number}: missing ':' separator."
                )
            ...
            if not value:
                ...
                try:
                    ...
                    raise BaseModBootstrapError(
                        f"Invalid localisation manifest at {path}:{line_number}: {exc.msg}"
                    ) from exc
            current[key] = decoded
    if current is not None:
        rows.append(current)
...
def _read_localization_manifest(path: Path) -> Tuple[LocalizationManifestEntry, ...]:
    rows = _parse_manifest_rows(path)
    entries: List[LocalizationManifestEntry] = []
    for row in rows:
        source = row.get("source", "")
        path_string = row.get("path", "")
        key = LocalizationKey(source=source, path=_parse_localization_path(path_string))
        translations = {
            language: text
            for language, text in row.items()
            if language not in LOCALIZATION_MANIFEST_FIXED_FIELDS
        }
        entry = LocalizationManifestEntry(
            key=key,
            category=row.get("category"),
            identifier=row.get("identifier"),
            field=row.get("field"),
            original_language=row.get("originalLanguage", "eng"),
            original_text=row.get("originalText", ""),
            translations=MappingProxyType(dict(translations)),
        )
        entries.append(entry)
    return tuple(entries)


def _set_nested_value(root: Any, path: Tuple[PathSegment, ...], value: str) -> Any:
    if not path:
        return value
    current = root
    parent: Any = None
    parent_key: Optional[PathSegment] = None
    for index, segment in enumerate(path):
        ...
        if isinstance(segment, int):
            if not isinstance(current, list):
                ...
                if parent is None:
                    ...
                    if isinstance(parent_key, int):
                        ...
                        parent[str(parent_key)] = new_list
                current = new_list
            ...
            if not isinstance(next_value, (dict, list)):
                ...
                next_value = [] if isinstance(next_key, int) else {}
            ...
            if not isinstance(current, dict):
                ...
                if parent is None:
                    ...
                    if isinstance(parent_key, int):
                        ...
                        assert isinstance(parent, dict)
                        parent[str(parent_key)] = new_dict
                current = new_dict
            if last:
                current[key] = value
                return root
...
class ModProject:
    ...
    def __init__(
        ...
        self.simple_card_blueprints: Dict[str, "SimpleCardBlueprint"] = {}
    ...
    def add_simple_card(self, blueprint: "SimpleCardBlueprint") -> None:
        ...
        self.simple_card_blueprints[blueprint.identifier] = blueprint
    ...
    def _collect_asset_localizations(
        self, resources_root: Path
    ) -> Tuple[Dict[LocalizationKey, Dict[str, str]], set[str]]:
        aggregated: Dict[LocalizationKey, Dict[str, str]] = {}
        languages: set[str] = set()
        localization_root = resources_root / "localizations"
        if not localization_root.exists():
            return aggregated, languages
        language_dirs = [entry for entry in localization_root.iterdir() if entry.is_dir()]
        for language_dir in sorted(language_dirs, key=lambda path: path.name.lower()):
            language_code = _normalise_language_code(language_dir.name)
            if not language_code:
                continue
            languages.add(language_code)
            json_files = sorted(language_dir.rglob("*.json"))
            for json_file in json_files:
                if not json_file.is_file():
                    continue
                try:
                    payload = json.loads(json_file.read_text(encoding="utf8"))
                except json.JSONDecodeError as exc:
                    raise BaseModBootstrapError(
                        f"Failed to parse localisation file {json_file}: {exc.msg}"
                    ) from exc
                relative = json_file.relative_to(language_dir)
                for path, text in _walk_localization_payload(payload):
                    if not isinstance(text, str):
                        continue
                    key = LocalizationKey(source=relative.as_posix(), path=tuple(path))
                    aggregated.setdefault(key, {})[language_code] = text
        return aggregated, languages

    def _merge_blueprint_localizations(
        self,
        aggregated: Dict[LocalizationKey, Dict[str, str]],
        languages: set[str],
        default_language: str,
    ) -> None:
        if not self.simple_card_blueprints:
            return
        from .cards import build_card_localizations

        payloads = build_card_localizations(
            self.simple_card_blueprints.values(), default_language=default_language or "eng"
        )
        for language, cards in payloads.items():
            language_code = _normalise_language_code(language)
            if not language_code:
                continue
            languages.add(language_code)
            for card_id, fields in cards.items():
                card_identifier = str(card_id)
                for field_name, value in fields.items():
                    ...
                    if isinstance(value, list):
                        for index, text in enumerate(value):
                            ...
                            key = LocalizationKey(
                                source="cards.json",
                                path=(card_identifier, field_label, index),
                            )
                            aggregated.setdefault(key, {}).setdefault(language_code, text)
                    else:
                        if not isinstance(value, str):
                            continue
                        key = LocalizationKey(
                            source="cards.json",
                            path=(card_identifier, field_label),
                        )
                        aggregated.setdefault(key, {}).setdefault(language_code, value)

    def _apply_localizations_to_resources(
        self,
        resources_root: Path,
        aggregated: Dict[LocalizationKey, Dict[str, str]],
    ) -> None:
        ...
        def _load_language_payload(language: str, source: str) -> Any:
            files = language_payloads.setdefault(language, {})
            if source in files:
                return files[source]  # type: ignore[return-value]
            source_path = localization_root / language / source
            if source_path.exists():
                try:
                    payload = json.loads(source_path.read_text(encoding="utf8"))
                except json.JSONDecodeError as exc:
                    raise BaseModBootstrapError(
                        f"Failed to parse localisation file {source_path}: {exc.msg}"
                    ) from exc
            else:
                payload = {}
        ...
        for key, language_map in aggregated.items():
            source = key.source or "cards.json"
            for language, text in language_map.items():
                language_code = _normalise_language_code(language)
                payload = _load_language_payload(language_code, source)
                updated = _set_nested_value(payload, key.path, text)
                if updated is not payload:
                    language_payloads[language_code][source] = updated

        for language, files in language_payloads.items():
            if not files:
                continue
            target_dir = localization_root / language
    ...
    def _build_localization_entries(
        ...
        aggregated: Dict[LocalizationKey, Dict[str, str]],
        base_language: str,
    ) -> Tuple[LocalizableTextEntry, ...]:
        items: List[LocalizableTextEntry] = []
        for key in sorted(
            aggregated,
            key=lambda entry: (entry.category(), entry.source, entry.as_path_string()),
        ):
            language_map = aggregated[key]
            if not language_map:
                continue
            original_language = base_language if base_language in language_map else next(iter(language_map))
            original_text = language_map[original_language]
            translations = {
                language: text
                for language, text in sorted(language_map.items())
                if language != original_language
            }
            items.append(
                LocalizableTextEntry(
                    key=key,
    ...
    def _load_existing_localization_overrides(
        self,
        options: BundleOptions,
        mod_root: Path,
        manifest_name: str,
    ) -> Dict[LocalizationKey, LocalizationManifestEntry]:
        overrides: Dict[LocalizationKey, LocalizationManifestEntry] = {}
        candidates: List[Path] = []
        assets_source = options.assets_source
        if assets_source:
            asset_manifest = assets_source / manifest_name
            parent_manifest = assets_source.parent / manifest_name
            for path in (parent_manifest, asset_manifest):
                if path not in candidates:
                    candidates.append(path)
        existing_manifest = mod_root / manifest_name
        if existing_manifest not in candidates:
            candidates.append(existing_manifest)

        for path in candidates:
            if not path.exists():
                continue
            for entry in _read_localization_manifest(path):
                key = entry.key
                ...
                merged.update(entry.translations)
                overrides[key] = LocalizationManifestEntry(
                    key=key,
                    category=entry.category or existing.category,
                    identifier=entry.identifier or existing.identifier,
                    field=entry.field or existing.field,
                    original_language=entry.original_language or existing.original_language,
                    original_text=entry.original_text or existing.original_text,
                    translations=MappingProxyType(dict(merged)),
                )
        return overrides

    def _generate_localization_manifest(
        self,
        options: BundleOptions,
        *,
        mod_root: Path,
        ...
        aggregated, languages = self._collect_asset_localizations(resources_root)
        default_language = "eng"
        base_candidate = "eng" if "eng" in languages else None
        self._merge_blueprint_localizations(aggregated, languages, base_candidate or default_language)

        for entry in overrides.values():
            key = entry.key
            if not key.source:
                continue
            original_language = _normalise_language_code(entry.original_language)
            if entry.original_text:
                aggregated.setdefault(key, {})[original_language or default_language] = entry.original_text
            if original_language:
                languages.add(original_language)
            for language, text in entry.translations.items():
                language_code = _normalise_language_code(language)
                languages.add(language_code)
                aggregated.setdefault(key, {})[language_code] = text

        if aggregated:
            base_language = "eng" if "eng" in languages else (sorted(languages)[0] if languages else default_language)
            for language_map in aggregated.values():
                if base_language not in language_map and language_map:
                    fallback_language, fallback_text = next(iter(language_map.items()))
                    language_map[base_language] = fallback_text
        else:
            base_language = "eng" if "eng" in languages else default_language

        if aggregated:
            self._apply_localizations_to_resources(resources_root, aggregated)
            entries = self._build_localization_entries(aggregated, base_language)
        else:
            entries = tuple()

        manifest_path = mod_root / manifest_name
        manifest_path.write_text(_serialise_localization_manifest(entries), encoding="utf8")
        self._localization_entries = entries
        PLUGIN_MANAGER.expose(
            f"mod_project:{self.mod_id}:localization_entries",
            self._localization_entries,
        )
    def _prepare_bundle_directory(self, options: BundleOptions) -> Path:
        output_dir = options.output_directory
        output_dir.mkdir(parents=True, exist_ok=True)
        mod_root_name = self.name.replace(" ", "")
        mod_root = output_dir / mod_root_name
        ...
        manifest_name = f"{mod_root_name}.yaml"
        existing_overrides = self._load_existing_localization_overrides(
            options, mod_root, manifest_name
        )
        if mod_root.exists():
        ...
        self._generate_localization_manifest(
            options,
            mod_root=mod_root,
...
PLUGIN_MANAGER.expose("LocalizableTextEntry", LocalizableTextEntry)
PLUGIN_MANAGER.expose("LocalizationManifestEntry", LocalizationManifestEntry)
